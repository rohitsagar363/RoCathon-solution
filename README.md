...
The only required submission file from this repo is:

```bash
submissions/brand_smart_home.json
```

Additional supporting artifacts generated by the repo are optional:
...
- Reranking uses a hybrid model over projected score, GMV, GPM, engagement, industry fit, query-specific industry fit, and audience fit
- The final blend stays close to the recommended baseline: `0.45` semantic / `0.55` business
- Hard penalties demote zero-GMV creators, zero-GPM creators, low-engagement creators, wrong-category matches, and candidates that miss the query's strongest anchor
